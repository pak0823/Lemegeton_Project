...
- **FindObjectsOfType 제거 (완료)**: `// [Optimization] Use registry` 주석 및 런타임 내 잔존 코드 17곳 모두 레지스트리 캐싱 방식으로 교체 완료
...
**[C-1] async void 패턴 — 예외 무음 처리 위험 (해결 완료)**

다음 5곳에 잔존하던 `async void` 호출을 `async UniTaskVoid`로 전환하고 `try-catch`로 래핑하여 개선했습니다.
...
**[C-2] 저장 시스템 일원화 및 안정화 (해결 완료)**

- 단일한 `Application.persistentDataPath` 기반 파일(`savedata.json`) 저장소로 통합 완료.
- `RuntimeUnitData.statModifiers` 등 Dictionary 직렬화가 누락되던 문제를 `ISerializationCallbackReceiver`를 활용한 커스텀 래퍼 구조로 구현하여 별도 패키지 추가 없이 로직 개선 완료.

**[C-3] CRLF 인코딩 혼용 (해결 완료)**

`PlayerMovement.cs`, `ATBTurnController.cs` 등에서 `\r\r\n` 패턴이 발견되던 문제를 개선했습니다.

- 프로젝트 전체에 `.gitattributes` 및 `.editorconfig` 생성 도입.
- 기존 잔존 스크립트의 찌꺼기 텍스트 인코딩(CRLF -> LF) 정제 및 UTF-8 강제 조치 완료.
...
| 파일                            | 줄 수          | 문제                                                                                                 |
| ------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------- |
| `PlayerMovement.cs` (해결 완료) | ~1,000줄       | 입력 처리와 상호작용 판정을 `ExplorationInteractionController` 로 통합 위임하여 기능 다운사이징 완료 |
...
-> `BattleManager.cs`, `BattleSkillProcessor.cs` 핵심 흐름을 UniTask로 통일 전환 완료.
...
**[I-2] 07_Test 폴더 분리 (해결 완료)**

`.asmdef`로 테스트 전용 어셈블리를 만들고 빌드에서 제외하는 설정이 필요하다.
-> `Lemegeton.Test.asmdef` 적용으로 빌드 제외 완료.

**[I-3] Addressables 핸들 관리 일원화 (해결 완료)**
...
| 우선순위     | 작업 항목                                                                                               | 예상 공수 |
| ------------ | ------------------------------------------------------------------------------------------------------- | --------- |
| P0 (완료)    | `async void` 5곳 → `async UniTaskVoid` + try-catch 전환                                                 | 완료      |
| P0 (완료)    | `.gitattributes` CRLF 설정 + 인코딩 일괄 정리                                                           | 완료      |
| P0 (완료)    | 저장 시스템 일원화 (PlayerPrefs → persistentDataPath JSON, statModifiers 직렬화 문제 해결)              | 완료      |
| 🟢 P1 (완료) | `FindObjectsOfType` 잔존 17곳 → `_activeUnits` 및 `PathfindingSystem` 레지스트리 기반 캐싱 교체 완료    | 완료      |
| P1 (완료)    | `PlayerMovement.cs` 책임 분리 (상호작용 및 입력 처리 `ExplorationInteractionController` 로직 이관 완료) | 완료      |
| � P1 (완료)  | `BattleStateMachine` FSM을 레거시 직접 상태 변경 방식(`BattleState` 할당)과 완전 통합 완료              | 완료      |
| 🟢 P1 (완료) | 개발용 플래그 `#if UNITY_EDITOR` 격리 완료                                                              | 완료      |
| 🟡 P2        | LuckySix `ShootingInsightPassive` TODO 3개 구현 완료                                                    | 2~3일     |
| 🔵 P3        | 단위 테스트 작성 (데미지 계산, ATB 순서, 인벤토리 로직)                                                 | 5일~      |
| 🔵 P3 (완료) | `07_Test` 폴더 `.asmdef` 분리 및 빌드 제외                                                              | 완료      |
| 🔵 P3 (완료) | Addressables 핸들 관리 ResourceTracker로 일원화                                                         | 완료      |
| 🔵 P3 (완료) | 비동기 패턴 Coroutine → UniTask 단계적 통일                                                             | 완료      |
