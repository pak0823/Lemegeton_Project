...

{
    ...
    private const string UnlockItemID = "107"; // [New] 해금 재화 아이템 ID (Manual)
    ...
    private void UpdateUnlockUI()
    {
        ...
        // [Mod] 재화 대신 아이템 개수 확인
        int currentAmount = InventoryManager.Instance.GetItemCount(UnlockItemID);

        // 텍스트 갱신: "보유량 / 필요량"
        // 색상 처리: 아이템 부족하면 빨간색
        string colorTag = currentAmount >= targetCost ? "<color=#00FF00>" : "<color=#FF0000>";
        if (currencyText) currencyText.text = $"{colorTag}{currentAmount}</color> / {targetCost}";

        // 버튼 활성/비활성 로직이 필요하다면 여기에 추가
    }
    ...
    {
        if (targetRouteIndex == -1 || InventoryManager.Instance == null) return;
        ...
        // [Mod] 아이템 소모 시도
        if (InventoryManager.Instance.ConsumeItem(UnlockItemID, targetCost))
        {
            // DB 해금
            if (TrainingDB.Instance != null)
            {
