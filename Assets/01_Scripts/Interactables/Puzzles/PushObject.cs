...
public class PushObject : MonoBehaviour //,IExplorationPersistable
{
    ...
    void Start()
    {
        // PathfindingSystem이 준비되면 초기 위치를 점유 타일로 등록
        if (PathfindingSystem.Instance != null && MainFloorMap != null)
        {
            _currentOccupiedCell = MainFloorMap.WorldToCell(transform.position);
    ...
    /// <summary>
    /// 상자가 밀려서 위치가 변경된 뒤 호출하여 점유 좌표를 갱신합니다.
    /// PlayerPushHandler의 PerformPush 완료 후 반드시 호출해야 합니다.
    /// </summary>
