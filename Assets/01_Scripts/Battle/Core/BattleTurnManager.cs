...
public class BattleTurnManager : MonoBehaviour
{
    ...
    IEnumerator EnemyTurnRoutine(BattleUnit enemy)
    {
        ...
        battleManager.EmitActionLabel(enemy, so.displayName);

        // 타겟 선정: AI에게 위임 (스킬에 맞는 최적 타겟)
        BattleUnit target = (ai != null) ? ai.SelectBestTarget(so) : null;
        ...
        if (target != null)
        {
            ...
            {
                // 타일 타겟 로직: 현재는 타겟 유닛의 위치를 타일로 간주
                yield return StartCoroutine(so.ResolveOnTile(battleManager, target.CurrentMap, target.Cell, enemy));
                battleManager.FinishActionAfterSkill();
                yield break;
            }
        }
        ...
        {
            // 타겟을 못 찾음 (모두 죽었거나 은신 등) -> 턴 종료
            Debug.Log($"[EnemyAI] {enemy.name} failed to find target for {so.displayName}");
            EndEnemyTurn(enemy);
        }
    }
    #endregion
