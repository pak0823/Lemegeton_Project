...
[CreateAssetMenu(menuName = "Battle/Passives/Shooting Insight", fileName = "Passive_ShootingInsight")]
public class ShootingInsightPassive : PassiveAsset
{
    [Header("Stack Config")]
    ...
    public override void OnDetach(BattleUnit owner, BattleManager battle)
    {
        if (owner == null) return;
        owner.OnDealtDamage -= HandleDealtDamage;
        _stacks.Remove(owner);
        // 추후 통찰 버프/상태를 쓴다면 여기서 제거
    }

    private void HandleDealtDamage(BattleUnit dealer, BattleUnit victim, int damage, SkillAsset source)
    {
        if (dealer == null) return;
        if (onlyOnDamagePositive && damage <= 0) return;

        if (!_stacks.TryGetValue(dealer, out var cur))
            cur = 0;

        cur++;
        Debug.Log($"[Passive:ShootingInsight] {dealer.name} stack -> {cur}");

        StatusController statusController = dealer.GetComponent<StatusController>();
        ...
        {
            Debug.Log($"[Passive:ShootingInsight] {dealer.name} 통찰 강화 발동 (x{insightValue}) [TODO: 실제 스탯 연동]");
        ...
        if (cur >= resetStacks)
        {
            Debug.Log($"[Passive:ShootingInsight] {dealer.name} 스택 리셋");
            cur = 0;
            if (statusController != null)
